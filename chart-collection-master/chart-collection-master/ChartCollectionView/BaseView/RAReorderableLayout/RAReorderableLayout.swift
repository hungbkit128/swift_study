...
open class RAReorderableLayout: UICollectionViewFlowLayout, UIGestureRecognizerDelegate {
    ...
    fileprivate func setUpGestureRecognizers() {
        ...
        longPress = UILongPressGestureRecognizer(target: self,
                                                 action: #selector(RAReorderableLayout.handleLongPress(_:)))
        panGesture = UIPanGestureRecognizer(target: self,
                                            action: #selector(RAReorderableLayout.handlePanGesture(_:)))
        longPress?.delegate = self
        panGesture?.delegate = self
        panGesture?.maximumNumberOfTouches = 1
        let gestures = collectionView.gestureRecognizers as NSArray?
