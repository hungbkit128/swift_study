...
//  CBCT-Viettel
//
//  Created by hnc on 2/1/18.
//  Copyright © 2018 Tung Duong Thanh. All rights reserved.
//

import UIKit

typealias Scale = (scaleW: Int, scaleH: Float)

protocol GridLayoutDelegate: class {
    func scaleForItem(inCollectionView collectionView: UICollectionView, withLayout layout: UICollectionViewLayout, atIndexPath indexPath: IndexPath) -> Scale
    func itemFlexibleDimension(inCollectionView collectionView: UICollectionView, withLayout layout: UICollectionViewLayout, fixedDimension: CGFloat, atIndexPath indexPath: IndexPath) -> CGFloat
    func headerFlexibleDimension(inCollectionView collectionView: UICollectionView, withLayout layout: UICollectionViewLayout, fixedDimension: CGFloat, section: Int) -> CGFloat
}
...
class GridLayout: UICollectionViewLayout, GridLayoutDelegate {
    ...
    private typealias ItemFrame = (section: Int, flexibleIndex: Int, fixedIndex: Int, scale: Scale)
    ...
    override func prepare() {
        ...
        if scrollDirection == .vertical {
            if isShowPaddingLeft == false {
                fixedDimension = collectionView.frame.width - (collectionView.contentInset.left + collectionView.contentInset.right) - 10
            } else {
                fixedDimension = collectionView.frame.width - (collectionView.contentInset.left + collectionView.contentInset.right) - 20
            }
        ...
        for section in 0 ..< collectionView.numberOfSections {
            let headerFlexibleDimension = (delegate ?? self).headerFlexibleDimension(inCollectionView: collectionView, withLayout: self, fixedDimension: fixedDimension, section: section)
            if divisionCounts.count == collectionView.numberOfSections {
                fixedDivisionCount = UInt(divisionCounts[section])
            }
            ...
            if headerFlexibleDimension > 0.0 {
            ...
            for item in 0 ..< itemCount {
                if fixedIndex >= fixedDivisionCount {
                ...
                itemFlexibleDimension = (delegate ?? self).itemFlexibleDimension(inCollectionView: collectionView, withLayout: self, fixedDimension: itemFixedDimension, atIndexPath: itemIndexPath)
                ...
                if (didFitInOriginalFrame) {
                    fixedIndex += 1 + itemFrame.scale.scaleW
                }
            }
        }
    ...
    // MARK: - Private
    private func indexableScale(forItemAt indexPath: IndexPath) -> Scale {
        var itemScale = (delegate ?? self).scaleForItem(inCollectionView: collectionView!, withLayout: self, atIndexPath: indexPath)
        if Int(fixedDivisionCount) == 2 && itemScale.scaleH == 3 && itemScale.scaleW == 3 { //case đặc biệt đối với bảng xu hướng khi show group dashboard
            itemScale.scaleW = 2
            itemScale.scaleH = 3
        } else {
            if itemScale.scaleH > Float(fixedDivisionCount) {
                itemScale.scaleH = Float(fixedDivisionCount)
            }
            if itemScale.scaleW > fixedDivisionCount {
                itemScale.scaleW = Int(fixedDivisionCount)
            }
        }
        return Scale(Int(itemScale.scaleW - 1), Float(itemScale.scaleH - 1))//Int(itemScale - 1) // Using with indices, want 0-based
    }
    ...
    private func nextAvailableFrame(startingAt originalFrame: ItemFrame) -> (frame: ItemFrame, fitInOriginalFrame: Bool) {
        ...
        while !isSpaceAvailable(for: newFrame) {
            ...
            if fixedIndex + originalFrame.scale.scaleW >= intFixedDivisionCount {
                fixedIndex = 0
                flexibleIndex += 1
            }
    ...
    private func isSpaceAvailable(for frame: ItemFrame) -> Bool {
        for flexibleIndex in frame.flexibleIndex ... frame.flexibleIndex + Int(frame.scale.scaleH) {
            ...
            for fixedIndex in frame.fixedIndex ... frame.fixedIndex + frame.scale.scaleW {
    ...
    private func reserveItemGrid(frame: ItemFrame) {
        for flexibleIndex in frame.flexibleIndex ... frame.flexibleIndex + Int(frame.scale.scaleH) {
            if flexibleIndex < sectionedItemGrid[frame.section].count {
                for fixedIndex in frame.fixedIndex ... frame.fixedIndex + frame.scale.scaleW {
    ...
    private func layoutAttributes(for indexPath: IndexPath, at itemFrame: ItemFrame, with sectionOffset: CGFloat) -> UICollectionViewLayoutAttributes {
        ...
        let itemScaledTransverseDimension = itemFixedDimension + (CGFloat(itemFrame.scale.scaleW) * (itemSpacing + itemFixedDimension))
        let itemScaledLongitudinalDimension = itemFlexibleDimension + (CGFloat(itemFrame.scale.scaleH) * (itemSpacing + itemFlexibleDimension))
        if scrollDirection == .vertical {
            if isShowPaddingLeft == false {
                ...
                layoutAttributes.frame = CGRect(x: fixedIndexOffset + 10, y: longitudinalOffset, width: itemScaledTransverseDimension, height: itemScaledLongitudinalDimension)
            }
