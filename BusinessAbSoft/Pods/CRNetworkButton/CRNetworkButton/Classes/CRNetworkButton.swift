...
open class CRNetworkButton: UIButton {
    ...
    fileprivate lazy var progressLayer: CAShapeLayer = {
        ...
        layer.path = UIBezierPath(arcCenter: self.boundsCenter, radius: self.boundsCenter.y - self.crLineWidth / 2, startAngle: CGFloat(-Double.pi/2), endAngle: 3*CGFloat(Double.pi/2), clockwise: true).cgPath
...
extension CRNetworkButton {
    ...
    fileprivate func dotsScalingAnimation() {
        ...
        let angleOffset = 2*CGFloat(Double.pi) / CGFloat(linesCount)
    ...
    fileprivate func pathForMark() -> UIBezierPath {
        ...
        let correctedPoint = CGPoint(x: boundsCenter.x + (boundsCenter.x * percentShiftX), y: boundsCenter.y + (boundsCenter.y * percentShiftY))
