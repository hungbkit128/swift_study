...
class GroupReportModel: NSObject {
    ...
    var isVTT : Bool = false
    var units: [ProvinceDistrictModel] = []
    ...
    init(groupShare: GroupDashboardSharedModel) {
        self.groupId = String(groupShare.groupId)
        self.groupName = groupShare.groupName
    }
    ...
    func setUnit (_ unit : [ProvinceDistrictModel]) {
        self.units.removeAll()
        self.units = unit
    }
    func isVTTDashboard() -> Bool {
        if dashboardType == "VTT" {
            return true
        }
        return false
    }
    
    func isGroupDashboardTD() -> Bool {
        return (self.groupType != .unknown)
            && (self.groupType == GroupDashboardType.aspectInfo
                || self.groupType == GroupDashboardType.tableAspect
                || self.groupType == GroupDashboardType.userAnalysticTD)
    }
}
